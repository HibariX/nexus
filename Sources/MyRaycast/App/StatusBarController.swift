    ...
    init() {
        ...
        if let button = statusItem.button {
            button.image = NSImage(systemSymbolName: "command.circle.fill",
                                   accessibilityDescription: "MyRaycast")
            button.image?.isTemplate = true
            button.contentTintColor = CyberpunkTheme.matrixNSColor
            button.setAccessibilityLabel("MyRaycast")
            button.target = self
        ...
        let quitItem = NSMenuItem(title: "退出 MyRaycast", action: #selector(NSApplication.terminate(_:)), keyEquivalent: "q")
